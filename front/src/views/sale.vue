<template>
  <div class="home">
    ...
    >
      <el-table-column prop="sale_id" label="销售流水号" width="120" sortable />
      <el-table-column prop="client_id" label="客户编号" />
      <el-table-column prop="goods_id" label="商品编号" />
      <el-table-column prop="sale_price" label="销售单价" />
      <el-table-column prop="sale_num" label="销售数量" />
      <el-table-column prop="sale_amount" label="销售金额" />
  ...
  <template slot-scope="{ row }">
    ...
    >
      ...
      <el-form :model="form" ref="form" label-width="100px">
        <!-- 1 -->
        <el-form-item label="销售流水号" prop="sale_id">
          <el-input
            v-model="form.sale_id"
            autocomplete="off"
            :placeholder="'请输入10位的销售流水号'"
            maxlength="10"
          />
        </el-form-item>
...
export default {
  ...
  methods: {
    init() {
      this.$axios.get("/all_sale").then((res) => {
        console.log(res);
        this.table = res.data;
      });
    },
    ...
    handDelete(index, row) {
      // 从行数据中提取出 id
      let id = JSON.parse(JSON.stringify(row)).sale_id; // 修改4

      // 发送 DELETE 请求删除指定 id 的数据
      this.$axios.delete(`/delete_sale?sale_id=${id}`).then((res) => {
    ...
    reset() {
      let id = undefined;
      if ("sale_id" in this.form) {
        id = this.form.sale_id;
      }
      this.form = {};
      if (id != undefined) this.form.sale_id = id;
    },
    save() {
      this.$refs["form"].validate((valid) => {
        ...
        if (valid) {
          ...
          if (!("new" in this.form)) {
            ...
            this.$axios.put("/update_sale", this.form).then((res) => {
              if (res.code == 200) {
                let _this = this;
                this.$notify.success({
                  title: "成功",
            ...
            console.log("添加");
            // this.form.dirty = 1;
            this.$axios.post("/add_sale", this.form).then((res) => {
              if (res.code == 200) {
                let _this = this;
                this.$notify.success({
                  title: "成功",
