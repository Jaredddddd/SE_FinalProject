<template>
  <div class="home">
    ...
    >
      <!-- <el-table :data="tables.slice((currentPage - 1) * pageSize, currentPage * pageSize)" border style="width: 100%" stripe ref="multipleTable" tooltip-effect="dark"> -->
      <el-table-column prop="goods_id" label="商品编号" width="100" sortable />
      <el-table-column prop="goods_name" label="商品名称" />
      <!-- <el-table-column prop="goods_num" label="商品数量" /> -->

      <el-table-column prop="goods_num" label="商品数量">
        <template slot-scope="scope">
          <span v-if="scope.row.goods_num < 10" style="color: red">{{
            scope.row.goods_num
          }}</span>
          <span v-else>{{ scope.row.goods_num }}</span>
        </template>
      </el-table-column>
    ...
    >
      <el-form :model="form" ref="form" label-width="70px">
        ...
        <!-- <el-form-item label="手机号码" prop="goods_num">
          <el-input
        ...
        <el-form-item label="商品数量" prop="goods_num">
          <el-input v-model="form.goods_num" autocomplete="off" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    init() {
      this.$axios.get("/all_goods").then((res) => {
        //修改
        console.log(res);
        this.table = res.data;
      });
    ...
    handDelete(index, row) {
      // 从行数据中提取出 id
      let id = JSON.parse(JSON.stringify(row)).goods_id; // 修改4

      // 发送 DELETE 请求删除指定 id 的数据
      this.$axios.delete(`/delete_goods?goods_id=${id}`).then((res) => {
    ...
    reset() {
      let id = undefined;
      if ("goods_id" in this.form) {
        id = this.form.goods_id;
      }
      this.form = {};
      if (id != undefined) this.form.goods_id = id;
    },
    save() {
      this.$refs["form"].validate((valid) => {
        ...
        if (valid) {
          ...
          if (!("new" in this.form)) {
            ...
            this.$axios.put("/update_goods", this.form).then((res) => {
              if (res.code == 200) {
                let _this = this;
                this.$notify.success({
                  title: "成功",
            ...
            console.log("添加");
            // this.form.dirty = 1;
            this.$axios.post("/add_goods", this.form).then((res) => {
              if (res.code == 200) {
                let _this = this;
                this.$notify.success({
                  title: "成功",
  ...
  computed: {
    ...
    hasInsufficientQuantity() {
      return this.table.some((item) => item.goods_num < 10);
    },
  },
};
</script>
