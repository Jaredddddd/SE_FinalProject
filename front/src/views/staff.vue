<template>
    <div class="home">
      ...
      >
        <el-table-column prop="staff_id" label="员工编号" width="100" sortable />
        <el-table-column prop="staff_name" label="员工姓名" />
        <el-table-column prop="department" label="工作部门" />
      ...
      >
        <el-form :model="form" ref="form" label-width="70px">
          ...
          <el-form-item label="员工姓名" prop="staff_name">
            <el-input v-model="form.staff_name" autocomplete="off" />
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      init() {
        this.$axios.get("/all_staff").then((res) => {
          console.log(res);
          this.table = res.data;
        });
      },
      ...
      handDelete(index, row) {
        // 从行数据中提取出 id
        let id = JSON.parse(JSON.stringify(row)).staff_id; // 修改4
  
        // 发送 DELETE 请求删除指定 id 的数据
        this.$axios.delete(`/delete_staff?staff_id=${id}`).then((res) => {
      ...
      reset() {
        let id = undefined;
        if ("staff_id" in this.form) {
          id = this.form.staff_id;
        }
        this.form = {};
        if (id != undefined) this.form.staff_id = id;
      },
      save() {
        this.$refs["form"].validate((valid) => {
          ...
          if (valid) {
            ...
            if (!("new" in this.form)) {
              ...
              this.$axios.put("/update_staff", this.form).then((res) => {
                if (res.code == 200) {
                  let _this = this;
                  this.$notify.success({
                    title: "成功",
              ...
              console.log("添加");
              // this.form.dirty = 1;
              this.$axios.post("/add_staff", this.form).then((res) => {
                if (res.code == 200) {
                  let _this = this;
                  this.$notify.success({
                    title: "成功",
      ...
      validateDepartment(rule, value, callback) {
        const validDepartments = ['purchase', 'sale'];

        if (!validDepartments.includes(value)) {
          callback(new Error('工作部门只能为"purchase"或"sale"'));
        } else {
          callback();
        }
      },
